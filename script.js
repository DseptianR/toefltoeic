...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (hbg && overlay) {
    /* Both click AND touchend for maximum iOS/Android compatibility */
    hbg.addEventListener('click', toggleNav);
    hbg.addEventListener('touchend', toggleNav, { passive: false });
  }

  if (closeB) {
    closeB.addEventListener('click', closeNav);
    closeB.addEventListener('touchend', function(e){ e.preventDefault(); closeNav(); }, { passive: false });
  }
